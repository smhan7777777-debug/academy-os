...
export async function startWebsiteSetup() {
  ...
  function render() {
    ...
    app.innerHTML = `<main class="design-studio" id="main">
      ...
      <section class="studio-included"><span class="eyebrow">CONNECTED TO YOUR OFFICE</span><h2>웹사이트를 함께 운영할 두 직원.</h2><div class="free-staff-grid">${FREE_STAFF.map((w) => `<article><span class="free-staff-symbol">${icon(w.id === "booking" ? "calendar" : "spark")}</span><div><h3>${w.name}</h3><p>${w.description}</p><span class="badge green">무료 제공</span></div></article>`).join("")}</div></section><footer class="app-footer"><span>배움결 · 웹사이트에서 시작하는 학원 운영</span><a href="/#website">상담·문의와 승인한 소식 관리 →</a></footer></main>`;
